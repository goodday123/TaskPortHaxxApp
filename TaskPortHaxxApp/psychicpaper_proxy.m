...
#pragma pack(pop)
        ...
        {
            req.state.__lr = __builtin_ptrauth_sign_unauthenticated(req.state.__lr, 0 /* ia */, 0);
            req.state.__opaque_flags &= ~__DARWIN_ARM_THREAD_STATE64_FLAGS_IB_SIGNED_LR;
            pc = (uint64_t)__builtin_ptrauth_sign_unauthenticated((void*)pc, 0 /* ia */, 0);
        }
#endif
...
//    NSLog(@"mach_port_insert_right: %s", mach_error_string(ret));
//    if(ret != KERN_SUCCESS) return MACH_PORT_NULL;
    ret = bootstrap_look_up(bootstrap_port, "com.kdt.taskporthaxx.exception_server", &strap_port);
    if(ret != KERN_SUCCESS) return MACH_PORT_NULL;
...
//    NSLog(@"task: 0x%x", task);
    ...
    // We can do this:
    *realport = task;
    sleep(1); // give it some time to update
